  ...
  if (loading) {
    return (
      <main
        ...
        style={{
          backgroundImage:
            "url('/nastuh-abootalebi-eHD8Y1Znfpk-unsplash.jpg')",
        }}
      >
    ...
    <div className="min-h-screen bg-slate-50 p-5 md:p-8">
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-5 lg:grid-cols-[260px_1fr]">
        ...
        <aside className="flex flex-col gap-4 lg:sticky lg:top-5 lg:self-start">
          ...
          <div className="flex items-center gap-4 rounded-2xl bg-card p-4 shadow-sm">
            <div className="flex shrink-0 flex-col items-center gap-1">
              <Image
                ...
                style={{ height: "auto" }}
              />
              <p className="text-xs uppercase leading-none tracking-wide text-muted">
                ShiftSync
              </p>
            </div>
          ...
          {/* Navigation */}
          <nav className="flex flex-col gap-1 rounded-2xl bg-card p-3 shadow-sm">
            <a
              href="/"
              className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-muted transition-colors hover:bg-slate-50 hover:text-foreground"
            >
            ...
            <a
              href="/audit"
              className="bg-primary/8 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-primary transition-colors"
              aria-current="page"
            >
              <span className="text-base">📋</span>
              Audit Trail
        ...
        <main className="space-y-5">
          <header className="rounded-2xl bg-card p-5 shadow-sm">
            ...
            <p className="mt-1 text-sm text-muted">
              All schedule changes — who, when, and what changed.
            </p>
          </header>
            ...
            <div className="flex flex-wrap items-end gap-3 border-b pb-5">
              ...
              {user?.role === "ADMIN" && (
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFilter((f) => ({ ...f, locationId: e.target.value }))
                    }
                  >
              ...
              <div className="ml-auto flex items-center gap-2">
                {user?.role === "ADMIN" && (
                  <button
                    onClick={exportCsv}
                    className="cursor-pointer rounded-md border px-3 py-1.5 text-sm hover:bg-slate-50"
                  >
                    Export CSV
                  </button>
                )}
                <button
                  ...
                  disabled={isFetching}
                  className="cursor-pointer rounded-md bg-primary px-4 py-1.5 text-sm text-white disabled:opacity-60"
                >
                  ...
                  <table className="w-full border-collapse text-xs">
                    ...
                    <tbody>
                      {logs.map((log) => {
                        ...
                        const actionColor = log.action.includes("DELETED") ||
                          log.action.includes("UNASSIGNED")
                          ? "text-danger"
                        ...
                        return (
                          ...
                          >
                            ...
                            <td
                              className={`py-2.5 pr-4 font-medium whitespace-nowrap ${actionColor}`}
                            >
                            ...
                            <td
                              className="max-w-[200px] truncate py-2.5 pr-4 text-muted"
                              title={summarise(before)}
                            >
                              {summarise(before)}
                            </td>
                            <td
            ...
            {total > PAGE_SIZE && (
              <div className="mt-4 flex items-center gap-3 border-t pt-4 text-sm">
                <button
                  ...
                  className="cursor-pointer rounded-md border px-3 py-1 disabled:opacity-40"
                >
                  ← Prev
                </button>
